...
def make_power_string(number: int, power: int) -> str:
    parts = [f"{c}^{power}" for c in str(number)]
    return " + ".join(parts)
...
def brute_force():
    ...
    for i in range(1000, 100000):
        (success, sum) = is_sum_of_factor_powers(i, factor)
        if success:
            print(
                f"{i:5d} is the sum {sum:5d} of the its digits to the fifth power {make_power_string(i, 5)}"
            )
